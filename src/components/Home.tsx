...
function Home({ usuarioLogado, onSair }: HomeProps) {
  ...
  const salvarViagem = async (evento: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const viagem = viagemEditando
        ? await atualizarViagem(viagemEditando.id, dados)
        : await cadastrarViagem(dados)
      setViagens((atuais) =>
        viagemEditando
          ? atuais.map((atual) => (atual.id === viagem.id ? viagem : atual))
          : [...atuais, viagem],
      )
      setNovaViagem({
        destino: "",
  ...
  const removerConta = async () => {
    const confirmou = window.confirm(
      "Excluir sua conta removerá também todas as suas viagens. Deseja continuar?",
    )
    if (!confirmou) return
  ...
  const salvarAtividade = async (evento: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const viagemAtualizada = {
        ...roteiroAberto,
        roteiro: atividadeEditando
          ? roteiroAberto.roteiro.map((item) =>
              item.id === atividade.id ? atividade : item,
            )
          : [...roteiroAberto.roteiro, atividade],
      }
      setViagens((atuais) =>
        atuais.map((viagem) =>
          viagem.id === roteiroAberto.id ? viagemAtualizada : viagem,
        ),
      )
      ...
      setMensagem(
        editando ? "Atividade atualizada." : "Atividade adicionada ao roteiro.",
      )
    } catch (err) {
      setErro(
        err instanceof Error ? err.message : "Erro ao adicionar atividade.",
      )
    } finally {
      setSalvando(false)
    }
  ...
  const removerAtividade = async (viagem: Viagem, atividade: RoteiroItem) => {
    try {
      ...
      const viagemAtualizada = {
        ...viagem,
        roteiro: viagem.roteiro.filter((atual) => atual.id !== atividade.id),
      }
      setViagens((atuais) =>
        atuais.map((item) => (item.id === viagem.id ? viagemAtualizada : item)),
      )
      setRoteiroAberto(viagemAtualizada)
    } catch (err) {
  ...
  const editarAtividade = (atividade: RoteiroItem) => {
    ...
    setNovaAtividade({
      ...
      data: /^\d{4}-\d{2}-\d{2}$/.test(atividade.data) ? atividade.data : "",
      hora: atividade.hora || "",
      categoria: atividade.categoria || "Passeio",
      local: atividade.local || "",
      custo: atividade.custo ? String(atividade.custo) : "",
  ...
  const proximaViagem =
    viagensOrdenadas.find((viagem) => viagem.dataFim >= hoje) ||
    viagensOrdenadas[0]
  const totalAtividades = viagens.reduce(
    (total, viagem) => total + viagem.roteiro.length,
    0,
  )
  const custoEstimado = viagens.reduce(
    (total, viagem) =>
      total +
      viagem.roteiro.reduce(
        (subtotal, atividade) => subtotal + (atividade.custo || 0),
        0,
      ),
    0,
  )
    ...
    <div className="home-page">
      <aside className="sidebar-panel">
        ...
        <div className="quick-card">
          ...
          <span>
            {viagens.length === 1 ? "viagem cadastrada" : "viagens cadastradas"}
          </span>
        </div>
      ...
      <main className="content-panel">
        <header className="topbar">
          ...
          <div className="user-actions">
            ...
            <div className="avatar-badge">
              {usuarioLogado.nome.slice(0, 1).toUpperCase()}
            </div>
          </div>
        </header>
          ...
          <div className="hero-copy">
            ...
            <p className="empty-copy">
              {viagens.length
                ...
                : "Sua agenda começa aqui. Cadastre uma viagem para acompanhar seus planos."}
            </p>
            <button
              className="hero-action"
              onClick={() => setModalAberto(true)}
            >
              {viagens.length ? "Planejar nova viagem" : "Cadastrar viagem"}
            </button>
          </div>
        ...
        <section className="stats-grid">
          ...
          <article className="stat-card">
            <p>Próxima viagem</p>
            <strong>
              {proximaViagem ? formatarData(proximaViagem.dataInicio) : "--"}
            </strong>
            <span>
              {proximaViagem ? proximaViagem.destino : "nenhuma cadastrada"}
            </span>
          </article>
          <article className="stat-card">
            <p>Perfil</p>
        ...
        <section className="travel-layout">
          <div className="destination-panel">
            ...
            {!carregando && !viagens.length && (
              <div className="empty-state">
                <strong>Nenhuma viagem cadastrada</strong>
                <span>
                  Use “Adicionar” para criar seu primeiro planejamento.
                </span>
              </div>
            )}
            {!carregando && viagens.length > 0 && !viagensFiltradas.length && (
              <div className="empty-state">
            ...
            {!carregando && viagensFiltradas.length > 0 && (
              <div className="destination-grid">
                {viagensFiltradas.map((viagem) => (
                  <article
                    key={viagem.id}
                    className="destination-card user-trip"
                  >
                    <div className="destination-overlay" />
                    <div className="destination-info">
                      <span>
                        {formatarData(viagem.dataInicio)} até{" "}
                        {formatarData(viagem.dataFim)}
                      </span>
                      <h4>{viagem.destino}</h4>
                      <div className="card-meta">
                        <strong>{viagem.roteiro.length} atividades</strong>
                        <button onClick={() => setRoteiroAberto(viagem)}>
                          Roteiro
                        </button>
                        <button onClick={() => abrirEdicao(viagem)}>
                          Editar
                        </button>
                        <button onClick={() => removerViagem(viagem)}>
                          Excluir
                        </button>
                      </div>
                    </div>
          ...
          <aside className="side-panel">
            <div className="panel-box">
              ...
              {proximaViagem ? (
                <>
                  <h4>{proximaViagem.destino}</h4>
                  <p>
                    {formatarData(proximaViagem.dataInicio)} até{" "}
                    {formatarData(proximaViagem.dataFim)}
                  </p>
                  <p>
                    {proximaViagem.observacoes ||
                      "Nenhuma observação adicionada."}
                  </p>
                  <button
                    className="text-button"
                    onClick={() => setRoteiroAberto(proximaViagem)}
                  >
                    Gerenciar roteiro
                  </button>
                </>
                ...
                <div className="empty-state">
                  <strong>Sem próxima viagem</strong>
                  <span>Cadastre uma viagem para vê-la aqui.</span>
                </div>
              )}
            </div>
            ...
            <div className="panel-box users-box">
              ...
              <button className="account-delete-button" onClick={removerConta}>
                Excluir conta
              </button>
            </div>
          </aside>
        ...
        <section className="itinerary-box" id="itinerario">
          ...
          <div className="itinerary-list">
            {!carregando && !viagens.length && (
              <div className="empty-state">
                <strong>Seu itinerário está vazio</strong>
                <span>As viagens cadastradas aparecerão nesta área.</span>
              </div>
            )}
            {viagens.map((viagem) => (
              <div key={viagem.id} className="itinerary-item">
                <div className="date-chip">
                  <strong>
                    {viagem.dataInicio.match(/^\d{4}-\d{2}-(\d{2})$/)?.[1] ||
                      "--"}
                  </strong>
                ...
                <button
                  className="text-button"
                  onClick={() => setRoteiroAberto(viagem)}
                >
                  Roteiro ({viagem.roteiro.length})
                </button>
              </div>
            ))}
          </div>
      ...
      {modalAberto && (
        ...
        >
          ...
          >
            <div className="section-heading">
              <h3>{viagemEditando ? "Editar viagem" : "Nova viagem"}</h3>
              <button
            ...
            <form onSubmit={salvarViagem}>
              ...
              <label>
                ...
                <textarea
                  ...
                  placeholder="Hotel, roteiro ou qualquer detalhe"
                />
              </label>
      ...
      {roteiroAberto && (
        <div
          className="modal-backdrop"
          onClick={() => {
            setRoteiroAberto(null)
            setAtividadeEditando(null)
          }}
        >
          <section
            className="trip-modal route-modal"
            onClick={(evento) => evento.stopPropagation()}
          >
            <div className="section-heading">
              <div>
                <p className="mini-label">Planejamento detalhado</p>
                <h3>Roteiro de {roteiroAberto.destino}</h3>
              </div>
              <button
                type="button"
                className="close-button"
            ...
            <div className="route-list">
              {!roteiroAberto.roteiro.length && (
                <div className="empty-state">
                  <strong>Nenhuma atividade</strong>
                  <span>
                    Adicione passeio, refeição, transporte ou hospedagem.
                  </span>
                </div>
              )}
              {roteiroAberto.roteiro.map((atividade) => (
                <div className="route-item" key={atividade.id}>
                  <div className="route-icon">
                    {(atividade.categoria || "P").slice(0, 1)}
                  </div>
                  <div className="route-details">
                    <strong>{atividade.titulo}</strong>
                    <small>
                      {atividade.categoria || "Passeio"} ·{" "}
                      {formatarData(atividade.data)}{" "}
                      {atividade.hora && `· ${atividade.hora}`}
                    </small>
                    {atividade.local && <span>Local: {atividade.local}</span>}
                    {atividade.detalhes && <span>{atividade.detalhes}</span>}
                    {atividade.custo > 0 && (
                      <span>
                        Custo estimado: R$ {atividade.custo.toFixed(2)}
                      </span>
                    )}
                  </div>
                  <div className="route-actions">
                    ...
                    <button
                      ...
                      onClick={() => removerAtividade(roteiroAberto, atividade)}
                    >
                      Excluir
                    </button>
                  </div>
            ...
            <form onSubmit={salvarAtividade}>
              <h4 className="form-title">
                {atividadeEditando ? "Editar atividade" : "Adicionar atividade"}
              </h4>
              <label>
                Nome da atividade
                <input
              ...
              <div className="form-row">
                ...
                <label>
                  ...
                  <input
                    required
                    type="date"
                    min={roteiroAberto.dataInicio}
                    max={roteiroAberto.dataFim}
              ...
              <div className="form-row">
                ...
                <label>
                  ...
                  <input
                    ...
                    min="0"
                    step="0.01"
                    value={novaAtividade.custo}
                    onChange={(evento) =>
                      setNovaAtividade({
                        ...novaAtividade,
                        custo: evento.target.value,
                      })
                    }
                    placeholder="0,00"
                  />
              ...
              <label>
                Detalhes
                <textarea
                  value={novaAtividade.detalhes}
                  onChange={(evento) =>
                    setNovaAtividade({
                      ...novaAtividade,
                      detalhes: evento.target.value,
                    })
                  }
                  placeholder="Reservas, lembretes ou informações importantes"
                />
              </label>
              {erro && <p className="erro">{erro}</p>}
              <button disabled={salvando}>
                {salvando
                  ...
                  : atividadeEditando
                    ? "Salvar alterações"
                    : "Adicionar atividade e continuar"}
              </button>
            </form>
          </section>
        </div>
